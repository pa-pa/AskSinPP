...
#define RFM69_OPMODE_TRANSMITTER 0x0C
  ...
  public:
    ...
    uint8_t interruptMode() { return FALLING; };
  ...
  public:
    ...
    bool init()
    {
      ...
      DPRINT(F("Boot Loader Freq: 0x21"));
      DHEX(f1);
      DHEXLN(f0);
#endif
      reset();
      ...
      DPRINT(F("CC Version: "));
      DHEXLN(spi.readReg(CC1101_VERSION, CC1101_STATUS));
  ...
  /// @brief Implementierung des RFM69
...
#ifdef USE_OTA_BOOTLOADER_FREQUENCY
              ,
              f1(0x00), f0(0x00) // set to defaults TODO
#endif
    {
    }
